...
class StarshipStatus:
    def __init__(self, update=False): #r = api response, either updated on object creation or taken from the data file
        if update == True:
            self.update()
        else:
...
Last Updated: {self.update_date}''')
    ...
    def request(self, tfr=False, weather=False, closures=False): #under construction, this will allow the user to request specific information with an easy function instead of calling like "data.tfrs[0].airspace['upper']"
        pass
